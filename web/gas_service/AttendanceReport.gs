function generateAttendanceReport(data) {
  try {
    ...
    if (header) {
      header.replaceText('{{MONTH_YEAR_TH}}', data.monthYear ? toThaiNumerals(data.monthYear) : '');
      header.replaceText('{{DIVISION_NAME}}', data.divisionName || 'ไม่ระบุกอง');
    }
    
    // เผื่อมีใน Body ด้วย
    body.replaceText('{{MONTH_YEAR_TH}}', data.monthYear ? toThaiNumerals(data.monthYear) : '');
    ...
    if (tables.length > 0 && employees.length > 0) {
      ...
      if (templateRow) {
        employees.forEach((emp, index) => {
          ...
          newRow.replaceText('{{date}}', emp.date ? toThaiNumerals(emp.date) : '');
          newRow.replaceText('{{sequence}}', toThaiNumerals(emp.sequence || String(index + 1)));
          newRow.replaceText('{{fullName}}', emp.fullName || '');
          newRow.replaceText('{{timeIn}}', emp.timeIn ? toThaiNumerals(emp.timeIn) : '');
          newRow.replaceText('{{timeOut}}', emp.timeOut ? toThaiNumerals(emp.timeOut) : '');
          newRow.replaceText('{{reason}}', emp.reason ? toThaiNumerals(emp.reason) : '');
    ...
    if (data.remarks && data.remarks.length > 0) {
      ...
      data.remarks.forEach(remark => {
        body.appendParagraph(toThaiNumerals(remark)).setAttributes({
          [DocumentApp.Attribute.FONT_SIZE]: 16,
